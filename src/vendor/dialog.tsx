 ...
 * @param deps
 */
const useIsomorphicLayoutEffect = /*#__PURE__*/ canUseDOM() ? react.useLayoutEffect : react.useEffect
const checkedPkgs = {}
/**
 ...
 *
 * @example
 * const ref = useCallbackRef(0, (newValue, oldValue) => logger.debug(oldValue, '->', newValue);
 * ref.current = 1;
 * // prints 0 -> 1
 *
...
const emptyArray = []
const FocusLock = /*#__PURE__*/ react.forwardRef(function FocusLockUI(props, parentRef) {
  let _extends2
...

  {
    ...
    react.useEffect(function () {
      if (!observed.current) {
        logger.error("FocusLock: could not obtain ref to internal node")
      }
    }, [])
  }
...
const getTopCommonParent = function (baseActiveElement, leftEntry, rightEntries) {
...
const activateTrap = function activateTrap() {
  ...
  if (lastActiveTrap) {
    ...
    if (workingNode) {
      const workingArea = [workingNode].concat(shards.map(extractRef$1).filter(Boolean))
...
const FocusLockCombination = /*#__PURE__*/ react.forwardRef(function FocusLockUICombination(props, ref) {
  return /*#__PURE__*/ react.createElement(
    FocusLock,
    _extends$1(
      {
...
const RemoveScroll = react.forwardRef(function (props, parentRef) {
  ...
  return react.createElement(
    ...
    forwardProps
      ? react.cloneElement(react.Children.only(children), __assign({}, containerProps, { ref: containerRef }))
      : react.createElement(
          Container,
...
const handleScroll = function (axis, endTarget, event, sourceDelta, noOverscroll) {
  ...
  react.useEffect(
    function () {
      if (props.inert) {
        ...
        const allow_1 = [props.lockRef.current].concat((props.shards || []).map(extractRef)).filter(Boolean)
...
const DialogOverlay = /*#__PURE__*/ forwardRefWithAs(function DialogOverlay(_ref, forwardedRef) {
...
const DialogContent = /*#__PURE__*/ forwardRefWithAs(function DialogContent(_ref3, forwardedRef) {
  const _ref3$as = _ref3.as,
    Comp = _ref3$as === void 0 ? "div" : _ref3$as,
    onClick = _ref3.onClick
  _ref3.onKeyDown
  const props = _objectWithoutPropertiesLoose(_ref3, ["as", "onClick", "onKeyDown"])
...
/** @type {any} */
const Dialog = /*#__PURE__*/ forwardRefWithAs(function Dialog(_ref4, forwardedRef) {
